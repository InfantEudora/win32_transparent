...
void ApplicationDozer::Init(){
    ...
    soundsystem->AppendFile("sound/engine_start_2.wav","engine_start");
    soundsystem->AppendFile("sound/arm_up.wav","arm_up");
    soundsystem->AppendFile("sound/engine_idle.wav","engine_idle");
    soundsystem->AppendFile("sound/engine_revup.wav","engine_revup");
    soundsystem->AppendFile("sound/engine_revup_long.wav","engine_revup_long");
    soundsystem->AppendFile("sound/engine_stop.wav","engine_stop");
    soundsystem->AppendFile("sound/engine_crank.wav","engine_crank");
    soundsystem->AppendFile("sound/steelbeam.wav","steelbeam");
    soundsystem->AppendFile("sound/door_opening.wav","door_opening");
...
Scene* ApplicationDozer::CreateMainScene(){
    ...
    gltfloader.LoadGLTFFile("meshes/dozer.glb");
    ...
    for (uint32_t i = 0; i < callbackData.getNbContactPairs(); i++) {
        ...
        if (beam && floor){
            ...
            if (contactPair.getEventType() == CollisionCallback::ContactPair::EventType::ContactStart){
                ...
                if (soundsystem->FinishedPlaying(snd_steelbeam)){
                    float gain;
                    ...
                    snd_steelbeam = soundsystem->Play("steelbeam",false,gain);
                    //debug->Info(" -> Doink!\n");
                }
            }
        }
