...
void DozerCharacter::MoveForward(){
    ...
    //Guarded so a held throttle does not layer rev-ups over each other. It CAN overlap now,
    //which is exactly why the guard has to name this playing rather than the sound.
    if (soundsystem->FinishedPlaying(snd_engine_revup)){
        snd_engine_revup = soundsystem->Play("engine_revup");
    }
...
void DozerCharacter::UpdatePhysicsState(){
    ...
    if (engine_state == ENGINE_STARTING){
        ...
        int stopped = soundsystem->FinishedPlaying(snd_engine_start);
        if (stopped){
            debug->Info("Engine start has stopped\n");
            engine_state = ENGINE_IDLE;
            //KEEP: it loops for as long as the engine runs, so it must survive every brick,
            //door and beam that plays over the top of it.
            snd_engine_idle = soundsystem->Play("engine_idle",true,1.0f,SOUND_KEEP);
        }
        r -= 0.5f;
        ...
        exhaust->morph_factors[0] = r;
        int stopped = soundsystem->FinishedPlaying(snd_engine_stop);
        if (stopped){
    ...
    if (arm_moving){
        ...
        if (arm_movement == 0.0f){
            arm_moving = false;
            //Stop rather than Pause: this gives the source back. The old Pause left it held
            //because the source belonged to the name for the life of the process anyway.
            soundsystem->Stop(snd_arm_up);
            snd_arm_up = SOUND_INVALID_HANDLE;
        }
        if (arm_movement < 0.1){
...
void DozerCharacter::ArmUp(){
    ...
    if (soundsystem && !arm_moving){
        //The Rewind that used to precede this is gone: a fresh voice starts at the beginning
        //by definition. It was only needed because the one source was still where the last
        //arm movement left it.
        snd_arm_up = soundsystem->Play("arm_up",false,1.0f,SOUND_KEEP);
        arm_moving = true;
        arm_torque = 0;
    }
    arm_movement = 0.5f;
...
void DozerCharacter::StartStopEngine(bool start){
    ...
    if (start && (engine_state == ENGINE_STOPPED)){
        snd_engine_start = soundsystem->Play("engine_start");
        engine_state = ENGINE_STARTING;
        smoke_emitter->EmitParticles(5);
    }else if (!start && (engine_state == ENGINE_IDLE)){
        snd_engine_stop = soundsystem->Play("engine_stop");
        //Stop, not Pause: the engine is off, and the looping idle should give its source up
        //rather than sit silently holding one until the next start.
        soundsystem->Stop(snd_engine_idle);
        snd_engine_idle = SOUND_INVALID_HANDLE;
