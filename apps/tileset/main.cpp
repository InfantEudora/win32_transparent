...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd){
    ...
    /*
        ...
        Note assets/icons is loaded as a FOLDER, by name, through Directory::GetFiles - the icon
        sheet is built from whatever *.png is in there rather than from a list. That goes through
        the same search path; see ResolveAssetDirectory in core/File.h.

        Relative to the EXECUTABLE, not the working directory - see GetExecutableDirectory in
        core/File.h.
    */
    AddAssetSearchRootFromExe("../assets");                 //apps/tileset/assets
