...
/*
    THE ANDROID HALF OF THE WATER. The desktop half is shaders/bomber_water.frag and the BODY OF
    THIS FILE IS THAT FILE, unchanged from `#define WATER_SHALLOW` downwards - the worley net, the
    exponential caustic falloff, the two water colours and every comment explaining them. Read it
    there; there is no second version of the reasoning to keep in step.

    --- WHAT HAD TO CHANGE, AND IT IS ONLY THE PREAMBLE --------------------------------------
    Unlike the blast (see bomber_explosion_android.frag, which is a reduction rather than a port),
    nothing this shader DOES is out of reach here. It reads two varyings and five floats, samples
    no texture and touches no buffer. Three lines of dialect stood in the way:

      - `#version 430 core`. This device compiles up to `320 es`; 310 is what the rest of this
        tree targets.
      - `layout (location = N) in ...` on FRAGMENT inputs. That is ES 3.2 (or the separate-shader
        extension), not core 3.1. Dropped entirely: default_android.vert publishes vposition and
        vnormal by NAME, which is how every other shader here is linked to it.
      - `uniform float water_scale = 4.0;` and its four siblings. GLSL ES has no uniform
        initialisers at all. Dropped, which is safe because ApplicationBomber::SetWaterUniforms
        pushes all five every frame - the defaults were documentation, not a fallback. THE DEFAULT
        VALUES ARE PRESERVED IN THE COMMENTS so the useful range of each knob is still written
        down next to it.

    So this is the one of bomber's two shader forks that is NOT permanent: if the desktop file
    ever drops to a version and a varying convention both platforms share, this file goes away and
    the pair becomes one. The blast fork does not have that future.
*/
...
//From default_android.vert, by name rather than by location - see the note above. Only these two
//are used; the rest of its varyings are still there.
...
uniform float water_scale;   //default 4.0
...
uniform float water_speed;   //default 1.0
...
uniform float water_depth;   //default 0.55
...
uniform float caustic_width;   //default 0.18

//Simulation ticks since the app started. See the clock note at the top.
uniform float water_time;   //default 0.0
