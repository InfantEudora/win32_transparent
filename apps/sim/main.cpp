...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd){
    ...
    /*
        ...
        assets/icons is loaded as a FOLDER by Directory::GetFiles - the resource sprite sheet is
        built from whatever *.png is in there rather than from a list. That goes through the same
        search path; see ResolveAssetDirectory in core/File.h.

        Relative to the EXECUTABLE, not the working directory - see GetExecutableDirectory in
        core/File.h.
    */
    AddAssetSearchRootFromExe("../assets");                 //apps/sim/assets
    AddAssetSearchRootFromExe("../../../shared_assets");    //default shaders and fonts

    Application* main_app = new ApplicationSim();
    main_app->Start();
