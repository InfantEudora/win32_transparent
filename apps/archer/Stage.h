...
private:
    ...
    //Moves the body box by `delta`, stopping against solid geometry, and reports what was hit.
    //Axis-separated: x first and resolved, then y - which is what makes running into a wall while
    //falling behave, and is the standard answer for a box platformer.
    void MoveAndCollide(const v2& delta, bool f_down_held, bool& out_hit_floor,
                        bool& out_hit_ceiling, bool& out_hit_wall);

    //True if the body box at `centre` overlaps anything solid. f_from_above decides whether
    //one-way platforms count.
    bool  BodyOverlapsSolid(const v2& centre, float prev_bottom, bool f_down_held) const;
