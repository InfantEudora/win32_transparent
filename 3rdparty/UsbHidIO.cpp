...
#include "UsbHidIO.h"
#include "Debug.h"
static Debugger* debug = new Debugger("UsbHidIO",DEBUG_INFO);
...
DWORD CUsbHidIO::SendData(PSP_DEVICE_INTERFACE_DETAIL_DATA aDetailData, PCHAR pData, USHORT len, HANDLE tmpWriteHandle){
	BOOL					ExternalHandle;
	...
	debug->Ok("Writing data ...\n");
	if (!DeviceIoControl(tmpWriteHandle, 0x8000a010, (LPVOID)pData, len, NULL, 0, NULL, NULL)){
		debug->Err("Failed\n");
	}
	//BOOLEAN res = HidD_SetOutputReport(tmpWriteHandle,(PVOID)pData,len);


	if (!ExternalHandle){
		CloseHandle(tmpWriteHandle);
	}
	return GetLastError();
}
								...
								HANDLE tmpWriteHandle)
{
	...
	*/
	...
	debug->Info("HidP_GetCaps: Capabilities.OutputReportByteLength = %i\n",Capabilities.OutputReportByteLength);
