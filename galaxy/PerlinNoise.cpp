...
int noise2(int x, int y, int seed){
    ...
    if (yindex < 0)
        yindex += 256;
    int  xindex = (HASH[yindex] + x + seed) % 256;
    if (xindex < 0)
        xindex += 256;
...
static double smooth_inter(double x, double y, double s){
    return lin_inter( x, y, fscurve3(s) );
}
...
float PerlinNoise::GetValue2D(float x, float y){
    float  xa = (x * frequency) + coord.x;
    float  ya = (y * frequency) + coord.y;
    float  current_persistence = 1.0;
    int this_seed = 0;
    ...
    for (int i=0; i<num_octaves; i++){
        ...
        signal = noise2d(xa, ya,this_seed);
    ...
    value = clamp(value * offset,0,255);
    value = clamp(value * scale,0,255);
    return value;
}