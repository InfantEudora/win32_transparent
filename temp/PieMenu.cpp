#include "PieMenu.h"
#include "Debug.h"
static Debugger *debug = new Debugger("PieMenu", DEBUG_INFO);
...
void EndPiePopup()
{
    ...
    for (int iIndex = 0; iIndex <= s_oPieMenuContext.m_iMaxIndex; ++iIndex)
    {
        ...
        float fMenuHeight = (float)sqrt(oPieMenu.m_fMaxItemSqrDiameter);
        ...
        for (int item_n = 0; item_n < oPieMenu.m_iCurrentIndex; item_n++)
        {
            ...
            for (int iSeg = 0; iSeg <= arc_segments; ++iSeg)
            {
                ...
                {
                    pDrawList->PrimWriteIdx((ImDrawIdx)pDrawList->_VtxCurrentIdx + 0);
...
void PieMenuExample(){
    debug->Info("IsWindowHovered = %d IsMouseClicked(1) = %d\n", ImGui::IsWindowHovered(), ImGui::IsMouseClicked(1));
    if (ImGui::IsMouseClicked(1))
    {
        ImGui::OpenPopup("PieMenu");
        debug->Info("Opening PieMenu\n");
    }

    if (BeginPiePopup("PieMenu", 1)){
        if (PieMenuItem("Push Object"))
        {
            debug->Info("Clicked Push Object\n");
        }

        if (PieMenuItem("Pick Object"))
        {
            debug->Info("Clicked Pick Object\n");
        }
        EndPiePopup();
    }
    ...
    if (BeginPiePopup("PieMenu", 1))
    {
        ...
        if (BeginPieMenu("Sub1"))
        {
            if (BeginPieMenu("Sub1.1\nmenu"))
            {
                if (PieMenuItem("Sub1.1.1"))
                {
                    // printf("Sub1.1.1\n");
                }
                if (PieMenuItem("Sub1.1.2"))
                {
                    // printf("Sub1.1.2\n");
                }
                EndPieMenu();
            }
            if (PieMenuItem("Sub1.2"))
            {
                // printf("Sub1.2\n");
            }
            if (PieMenuItem("Sub1.3"))
            {
                // printf("Sub1.3\n");
            }
            EndPieMenu();
        ...
        {
            if (PieMenuItem("Sub2.1"))
            {
                // printf("Sub2.1\n");
            }
            if (BeginPieMenu("Sub2.2\nmenu"))
            {
                if (PieMenuItem("Sub2.2.1"))
                {
                    // printf("Sub2.2.1\n");
                }
                if (PieMenuItem("Sub2.2.2"))
                {
                    // printf("Sub2.2.2\n");
                }
                EndPieMenu();
            }
            if (PieMenuItem("Sub2.3"))
            {
                // printf("Sub2.3\n");
            }
            EndPieMenu();
