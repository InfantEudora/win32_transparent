...
Tank app tools (`ApplicationTank::RegisterMCPTools()`, called from `ApplicationTank::Init()`): `tank_drive`, `tank_steer`, `tank_telemetry`, `tank_screenshot`, `tank_pause`, `tank_step`, `bridge_*`, `crane_speed` (`speed` for the boom hinge motor and/or `extension_speed` for the telescoping extension's slider motor, `hook_speed` for the hook's winch - all -1..1 velocity commands), `crane_telemetry`.
