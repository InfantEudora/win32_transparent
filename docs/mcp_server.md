...
`tools/baseline_handrolled_*.json` are the runs recorded against the original in-game raycast suspension (`core/Wheel.cpp` up to commit `b4d0b70`) and `tools/baseline_rp3d_*.json` the same scenario right after the wheels moved into rp3d's `VehicleConstraint`. Both were recorded with the scenario as it is in `vehicle_mcp.py`, so `compare` between them is apples to apples. Highlights: identical ride height (11.1 vs 11.2 mm for the tank); the tank now reaches its top speed, stops under the brake (before: 0.49 m/s left over with the nose lifted) and pivots at 1.4 rad/s instead of 8.9; the buggy brakes to a standstill level instead of flipping, and no longer keeps driving after "stop" (that was a `ReleaseInputs` bug that never released the throttle - the tank masked it by zeroing its own gas pedal every tick).
