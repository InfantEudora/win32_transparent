...
{
	m_info.powerActiveImport = powerWatts;
	m_info.soc = socPercent;

	json sampledValue = json::array();

	// Power value
	sampledValue.push_back({
		{"value", std::to_string(powerWatts)},
	...
	// SoC value
	sampledValue.push_back({
		{"value", std::to_string(socPercent)},
		{"context", "Sample.Periodic"},
		{"measurand", "SoC"},
		{"unit", "Percent"}
	});
...
std::string OCPPClient::DecodeWebSocketFrame(const char* data, int length, int& bytesConsumed)
{
	...
	if (length < offset + static_cast<int>(payloadLen))
...
void OCPPClient::HandleOCPPMessage(const std::string& message){
	...
	else if (messageType == 2)  // CALL (server request)
	{
		...
		// Send a simple acceptance response for any server request
