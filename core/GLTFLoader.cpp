...
void GLTFLoader::LoadGTLFFile(const char* input_filename){
...
vertex GLTFLoader::GetVertex(tinygltf::BufferView* pb, tinygltf::BufferView* nb, tinygltf::BufferView* ub, int index){
    ...
    v.normal = Getvec3(&normal_buffer.data.at(0),byte_offset_position);
    v.uv = Getvec2(&uv_buffer.data.at(0),byte_offset_position);
...
Mesh* GLTFLoader::GetMeshFromNode(const char* node_name){
