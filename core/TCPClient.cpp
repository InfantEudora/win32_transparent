...
TCPClient::TCPClient()
    : m_clientSocket(INVALID_SOCKET), m_connected(false),
...
bool TCPClient::Connect(const std::string &host, int port) {
    ...
    m_receiveThread = CreateThread(NULL, 0, ReceiveDataThread, (LPVOID)this, 0, NULL);

    if (!m_receiveThread) {
        debug->Err("CreateThread failed: %d\n", GetLastError());
        m_connected = false;
        closesocket(m_clientSocket);
        m_clientSocket = INVALID_SOCKET;
        return false;
